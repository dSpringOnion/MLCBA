...
SAMPLE_VIDEOS_FOLDER = os.path.join('backend', 'sample_videos')
...
def load_models():
    ...
    try:
        ...
        elif os.path.exists('behavior_model.pkl'):
            classifier.load_model('behavior_model.pkl')
            print("Loaded pre-trained model from root")
        else:
...
def home():
    return jsonify({
        ...
        'models_loaded': models_loaded,
        'loading_error': loading_error,
        'endpoints': ['/health', '/status', '/upload', '/process_frame', '/sample_videos', '/process_sample', '/processed_video', '/cleanup_video', '/retrain_model']
    })
...
def upload_video():
    ...
    try:
        ...
        if not models_loaded or not detector or detector.model is None:
            return jsonify({
                'error': 'AI models are still loading. Please wait a few minutes and try again.',
